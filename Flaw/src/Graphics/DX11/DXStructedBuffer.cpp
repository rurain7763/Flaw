...
namespace flaw {
	...
	void DXStructuredBuffer::BindToGraphicsShader(const uint32_t slot) {
		if (_unbindFunc) {
			Log::Warn("You try to bind already bound structured buffer");
			return;
		}
	...
	void DXStructuredBuffer::BindToComputeShader(const BindFlag bindFlag, const uint32_t slot) {
		if (_unbindFunc) {
			Log::Warn("You try to bind already bound structured buffer");
			return;
		}
