...
namespace flaw {
	...
	public:
		...
		static void DrawCircle(const uint32_t id, const mat4& transform, const vec4& color, const float thickness = 1.0);

		// TODO: add thickness
		static void DrawLine(const uint32_t id, const vec3& start, const vec3& end, const vec4& color);

		static void DrawLineRect(const uint32_t id, const mat4& transform, const vec4& color);
